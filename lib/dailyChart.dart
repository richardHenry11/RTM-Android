...
class _ChartDailyState extends State<ChartDaily> {
  ...
  void initState() {
    ...
    socketService.onStatusChange = (status) {
      ...
      if (status == "connected") {
        ...
        isReconnecting = true;
        print("⚠️ WebSocket Disconnected, retrying in 1 seconds...");
  ...
  Future<void> _chartGetter() async {
    if (_email == null) {
      print("Email hasnt been loaded, try again");
      return;
    }
    ...
    if (res.statusCode == 200) {
      if (res.body.trim() == 'null') {
        ...
        // Tampilkan notifikasi atau snackbar
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Chart data is not available."),
...
List<LineChartBarData> getChartBarsFromGroup(Map<String, List<FlSpot>> group) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Daily Chart", style: TextStyle(color: Colors.white)),
