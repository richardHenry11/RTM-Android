...
class ChartWeekly extends StatefulWidget {
  const ChartWeekly({super.key});

  @override
  State<ChartWeekly> createState() => _ChartWeeklyState();
}
...
class _ChartWeeklyState extends State<ChartWeekly> {
  ...
  Map<double, String> indexToDate = {}; // untuk label X axis (tanggal)
      ...
      if (data.containsKey("status") && data["status"] == "Stop") {
        if (data.containsKey("running_time_float")) {
          ...
          setState(() {
            ...
            // Tambahkan data ke grup berdasarkan device_name
            chartDataMap.putIfAbsent(deviceName, () => []);
            chartDataMap[deviceName]!.add(FlSpot(index, timeFloat));

            deviceNames[index] = deviceName; // Simpan device_name untuk tooltip
          });
        }
      }
    } catch (e, stackTrace) {
  ...
  Future<void> _chartGetter() async {
    ...
    if (res.statusCode == 200) {
      ...
      // Kumpulkan semua tanggal unik dari semua device
      Set<String> allDates = {};
      for (var device in data) {
        List<dynamic> entries = device['data'] ?? [];
        for (var entry in entries) {
          if (entry['week_start'] != null) {
            allDates.add(entry['week_start']);
          }
        }
      }

      // Urutkan tanggal-tanggal secara global
      ...
      print("❌ Gagal mengambil data chart, code: ${res.statusCode}");
    }
  }
  ...
  final allY = group.values.expand((list) => list).map((e) => e.y).toList();
  if (allY.isEmpty) return 0;
  return allY.reduce((a, b) => a < b ? a : b);
}

double getMaxYFromGroup(Map<String, List<FlSpot>> group) {
  final allY = group.values.expand((list) => list).map((e) => e.y).toList();
  if (allY.isEmpty) return 6;
  return allY.reduce((a, b) => a > b ? a : b);
}
...
Widget buildLegend(Map<String, List<FlSpot>> group) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Weekly Chart", style: TextStyle(color: Colors.white)),
