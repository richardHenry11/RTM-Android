...
class _RawDataState extends State<RawData> {
  ...
  Future<void> exportToPDF(List<List<String>> data, BuildContext context) async {
      ...
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.TableHelper.fromTextArray(
            headers: ["Site ID", "Device ID", "Sensor ID", "Current1", "Power1", "energy1", "Frequency1", "PF1", "MAC", "BATT", "RSSI", "Created At", "Updated At", "Start Time", "End Time", "Status", "Volatage"],
    ...
    final downloadDir = Directory('${directory!.path}/Download/rawData');
    ...
    final filePath = "${directory!.path}/Download/rawData/data_export.pdf";
  ...
  Future<void> exportToExcel(List<List<String>> data) async {
    ...
    Sheet sheet = excel['sheet1'];
     ...
     // Add header
  sheet.appendRow(["Site ID", "Device ID", "Sensor ID", "Current1", "Power1", "energy1", "Frequency1", "PF1", "MAC", "BATT", "RSSI", "Created At", "Updated At", "Start Time", "End Time", "Status", "Volatage"].map((e) => TextCellValue(e)).toList());
  ...
  if (fileBytes != null) {
    ...
    String downloadsPath = "${downloadsDir!.path}/Download/rawData"; // Folder Download
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Raw Data", style: TextStyle(color: Colors.white)),
      ),
      body:
      SafeArea(
        child:
              ...
              Row(
                ...
                Expanded(
                  ...
                  ElevatedButton(
                    ...
                    if (tableData.isEmpty) {
                      print("Data kosong saat diekspor!");
                      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    // Debugging: Print data sebelum ekspor
                    print("Data yang diekspor: $tableData");

                    List<List<String>> excelData = tableData.map((row) => [
                      row["Site ID"].toString(),
                      row["Device ID"].toString(),
                      row["Sensor ID"].toString(),
                      row["Current1"].toString(),
                      row["Power1"].toString(),
                      row["energy1"].toString(),
                      row["Frequency1"].toString(),
                      row["PF1"].toString(),
                      row["MAC"].toString(),
                      row["BATT"].toString(),
                      row["RSSI"].toString(),
                      row["Created At"].toString(),
                      row["Updated At"].toString(),
                      row["Start Time"].toString(),
                      row["End Time"].toString(),
                      row["Status"].toString(),
                      row["Volatage"].toString(),
                    ]).toList();
               ...
               Padding(
                ...
                child: ElevatedButton(
                  ...
                  child: Text("Excel", style: TextStyle(color: Colors.white)),
                ),
              ),
            ],
          ),
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    columns: const [
                      ...
                      DataColumn(label: Text("Current1")),
                      DataColumn(label: Text("Power1")),
                      DataColumn(label: Text("Energy1")),
                      DataColumn(label: Text("Frequency1")),
                      ...
                      DataColumn(label: Text("Volatage")),
                    ],
                    rows: tableData.map((data) {
                      return DataRow(cells: [
                        DataCell(Text(data["Site ID"] ?? "N/A")),
                        DataCell(Text(data["Device ID"] ?? "N/A")),
                        DataCell(Text(data["Sensor ID"] ?? "N/A")),
                        DataCell(Text(data["Current1"] ?? "N/A")),
                        DataCell(Text(data["Power1"] ?? "N/A")),
                        DataCell(Text(data["Energy1"] ?? "N/A")),
                        DataCell(Text(data["Frequency1"] ?? "N/A")),
                        DataCell(Text(data["PF1"] ?? "N/A")),
                        DataCell(Text(data["MAC"] ?? "N/A")),
                        DataCell(Text(data["BATT"] ?? "N/A")),
                        DataCell(Text(data["RSSI"] ?? "N/A")),
                        DataCell(Text(data["Created At"] ?? "N/A")),
                        DataCell(Text(data["Updated At"] ?? "N/A")),
                        DataCell(Text(data["Start Time"] ?? "N/A")),
                        DataCell(Text(data["End Time"] ?? "N/A")),
                        DataCell(Text(data["Status"] ?? "Tidak ada status")),
                        DataCell(Text(data["Volatage"] ?? "N/A")),
                      ]);
                    }).toList(),
                  ),
                ),
